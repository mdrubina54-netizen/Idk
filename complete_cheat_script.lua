-- Complete Cheat Script with GUI
...
local phem_UserInputService = game:GetService("UserInputService")
...
-- Tracer Line
local phem_tracer = Drawing.new("Line")
...
-- RenderStepped for FOV, Tracer, and Aimlock
phem_RunService.RenderStepped:Connect(function()
    ...
    if phem_target and getgenv().Config.ShowTracer then
...
phem7.Raycast = function(self, phem19, phem20, phem21, phem22, phem23, phem24)
    ...
    if getgenv().Config.BodyHitChance < 100 and math.random(1, 100) > getgenv().Config.BodyHitChance then
        local phem_bodyParts = phem_silentTarget.Parent:FindFirstChild("Humanoid") and phem_silentTarget.Parent:GetDescendants() or {}
        local phem_validParts = {}
        for _, part in pairs(phem_bodyParts) do
            if part:IsA("BasePart") and part ~= phem_silentTarget then
                table.insert(phem_validParts, part)
            end
        end
        if #phem_validParts > 0 then
            phem_bodyHit = phem_validParts[math.random(1, #phem_validParts)]
        end
    end
    ...
    phem_RunService.Heartbeat:Connect(function()
        ...
        if (tick() - phem_lastFire) > phem_fireRate then
            ...
            if phem_target then
                ...
                local phem_weaponRemote = phem5:FindFirstChild("Weapon")
                ...
                if phem_weaponRemote then pcall(function() phem_weaponRemote:FireServer() end) end
...
function phem_UpdateTabContent()
    ...
    if phem_CurrentTab == "Aimbot" then
        ...
        phem_CreateToggle(phem_ContentFrame, "Katana Check", "KatanaCheck")
    ...
    elseif phem_CurrentTab == "Visual" then
        ...
        phem_CreateCombobox(phem_ContentFrame, "ESP Line Position", "ESPLinePos", {"Top", "Bottom", "Side", "Center"})
        phem_CreateToggle(phem_ContentFrame, "ESP Box", "ESPBox")
        phem_CreateCombobox(phem_ContentFrame, "ESP Box Type", "ESPBoxType", {"Solid", "Corner", "Filled"})
        phem_CreateToggle(phem_ContentFrame, "ESP Health Bar", "ESPHealthBar")
        phem_CreateCombobox(phem_ContentFrame, "Health Bar Position", "ESPHealthPos", {"Left", "Right", "Top", "Bottom"})
        phem_CreateToggle(phem_ContentFrame, "ESP Name", "ESPName")
        phem_CreateCombobox(phem_ContentFrame, "Name Position", "ESPNamePos", {"Top", "Bottom"})
        phem_CreateToggle(phem_ContentFrame, "ESP Distance", "ESPDistance")
        phem_CreateCombobox(phem_ContentFrame, "Distance Position", "ESPDistancePos", {"Top", "Bottom"})
    ...
    elseif phem_CurrentTab == "Extra" then
        ...
        phem_CreateToggle(phem_ContentFrame, "Magnet Lite (Side+Up)", "MagnetLite")
        ...
        phem_CreateToggle(phem_ContentFrame, "Teleport to Nearest", "TeleportNearest")
    end
end

phem_UpdateTabContent()

print("✓ Complete Cheat Script Loaded")
print("Silent Aim + Aimlock + Magnet Pull Active")
print("Press X to toggle menu visibility")
