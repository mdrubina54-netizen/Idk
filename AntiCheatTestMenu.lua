-- ================================================
--   AntiCheat Test Menu v4
--   Fixed: ScreenGui always visible, safe parent
-- ================================================
...
local function getGuiParent()
    ...
    -- Try gethui (Fluxus / some others)
    if gethui then
        local sg = Instance.new("ScreenGui")
        sg.Parent = gethui()
        return sg
    end
    -- Try CoreGui directly
    ...
    -- Final fallback: PlayerGui (always works)
    local sg = Instance.new("ScreenGui")
    sg.Parent = LocalPlayer:WaitForChild("PlayerGui")
    return sg
end

local SG = getGuiParent()
SG.Name           = "ACMenuV4"
SG.ResetOnSpawn   = false
SG.IgnoreGuiInset = true
SG.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
SG.DisplayOrder   = 999  -- always on top
...
local function initESP(p)
    ...
    Pool[p] = {
        ...
        dist = newDraw("Text",  {Visible=false,Size=12,Color=Color3.fromRGB(255,255,180),Center=true,Outline=true,OutlineColor=Color3.new(0,0,0)}),
    }
end
...
local function getScreenBounds(char, root)
    ...
    if not found then
        ...
        if vis and depth > 0 then return sp.X-20,sp.Y-50,sp.X+20,sp.Y+10,true end
...
local function getBestTarget()
    ...
    for _,p in ipairs(Players:GetPlayers()) do
        ...
        local res = {getCharInfo(p)}
        if not res[1] then continue end
        local char,root,head,hum = res[1],res[2],res[3],res[4]
...
WIN.Name             = "MainWindow"
WIN.Size             = UDim2.new(0,430,0,390)
WIN.Position         = UDim2.new(0,60,0,60)   -- top-left, always visible
...
BAR.Name="TitleBar" BAR.Size=UDim2.new(1,0,0,34)
BAR.BackgroundColor3=T.bar BAR.BorderSizePixel=0 BAR.ZIndex=11
Instance.new("UICorner",BAR).CornerRadius=UDim.new(0,8)
-- cover lower rounded corners of bar
local cov=Instance.new("Frame",BAR)
cov.Size=UDim2.new(1,0,0,10) cov.Position=UDim2.new(0,0,1,-10)
cov.BackgroundColor3=T.bar cov.BorderSizePixel=0 cov.ZIndex=11
...
local function mkCtrlBtn(xOff,bg,lbl)
    ...
    b.Font=Enum.Font.GothamBold b.TextSize=11 b.BorderSizePixel=0 b.ZIndex=13
    Instance.new("UICorner",b).CornerRadius=UDim.new(0,4)
    return b
end
local CBTN = mkCtrlBtn(-6, T.close,"✕")
local MBTN = mkCtrlBtn(-32,T.mini, "─")

-- Drag
local drag,dragOrig,mOrig=false,nil,nil
BAR.InputBegan:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.MouseButton1 then
        drag=true dragOrig=WIN.Position mOrig=Vector2.new(i.Position.X,i.Position.Y)
    end
end)
...
UserInputService.InputChanged:Connect(function(i)
    if drag and i.UserInputType==Enum.UserInputType.MouseMovement then
        local d=Vector2.new(i.Position.X,i.Position.Y)-mOrig
        WIN.Position=UDim2.new(0,dragOrig.X.Offset+d.X,0,dragOrig.Y.Offset+d.Y)
    end
end)
...
MBTN.MouseButton1Click:Connect(function()
    ...
    WIN.Size=minimized and UDim2.new(0,430,0,34) or UDim2.new(0,430,0,390)
...
TABROW.Size=UDim2.new(1,-16,0,30) TABROW.Position=UDim2.new(0,8,0,38)
TABROW.BackgroundTransparency=1 TABROW.BorderSizePixel=0 TABROW.ZIndex=11

-- Scroll area
local CLIP=Instance.new("Frame",WIN)
CLIP.Size=UDim2.new(1,-16,1,-80) CLIP.Position=UDim2.new(0,8,0,74)
CLIP.BackgroundTransparency=1 CLIP.BorderSizePixel=0
CLIP.ClipsDescendants=true CLIP.ZIndex=10
...
local function addTab(name)
    ...
    sf.CanvasSize=UDim2.new(0,0,0,0) sf.Visible=false sf.ZIndex=10
...
local cursors={}
local function cy(p) cursors[p]=cursors[p] or PAD return cursors[p] end
local function ay(p,h)
    cursors[p]=(cursors[p] or PAD)+h+4
    p.CanvasSize=UDim2.new(0,0,0,cursors[p]+10)
end

local function addSection(page,lbl)
    local y=cy(page)
    local f=Instance.new("Frame",page)
    f.Size=UDim2.new(1,-4,0,SH) f.Position=UDim2.new(0,2,0,y) f.ZIndex=11
    f.BackgroundColor3=T.accent f.BackgroundTransparency=0.78 f.BorderSizePixel=0
    Instance.new("UICorner",f).CornerRadius=UDim.new(0,5)
    local l=Instance.new("TextLabel",f)
    l.Size=UDim2.new(1,-12,1,0) l.Position=UDim2.new(0,10,0,0) l.ZIndex=12
...
local function baseRow(page)
    local y=cy(page)
    local f=Instance.new("Frame",page)
    f.Size=UDim2.new(1,-4,0,IH) f.Position=UDim2.new(0,2,0,y) f.ZIndex=11
...
local function rlabel(f,txt)
    local l=Instance.new("TextLabel",f)
    l.Size=UDim2.new(1,-60,1,0) l.Position=UDim2.new(0,10,0,0) l.ZIndex=12
...
local function addToggle(page,lbl,key,cb)
    ...
    tog.Text="" tog.BorderSizePixel=0 tog.ZIndex=13
    Instance.new("UICorner",tog).CornerRadius=UDim.new(1,0)
    local knob=Instance.new("Frame",tog)
    knob.Size=UDim2.new(0,14,0,14) knob.BorderSizePixel=0 knob.ZIndex=14
...
local function addDropdown(page,lbl,key,opts,cb)
    ...
    btn.TextSize=11 btn.BorderSizePixel=0 btn.ZIndex=13
...
local function addSlider(page,lbl,key,mn,mx,cb)
    local y=cy(page)
    local f=Instance.new("Frame",page)
    f.Size=UDim2.new(1,-4,0,SLTH) f.Position=UDim2.new(0,2,0,y) f.ZIndex=11
    f.BackgroundColor3=T.panel f.BorderSizePixel=0
    Instance.new("UICorner",f).CornerRadius=UDim.new(0,6)
    local st=Instance.new("UIStroke",f) st.Color=T.border st.Thickness=1
    ay(page,SLTH)
    local l=Instance.new("TextLabel",f)
    l.Size=UDim2.new(1,-10,0,18) l.Position=UDim2.new(0,10,0,4) l.ZIndex=12
    ...
    track.BackgroundColor3=T.slbg track.BorderSizePixel=0 track.ZIndex=12
    ...
    knob.Position=UDim2.new(pct,0,0.5,0) knob.ZIndex=13
...
local function addInfo(page,txt)
    local f=baseRow(page)
    local l=Instance.new("TextLabel",f)
    l.Size=UDim2.new(1,-10,1,0) l.Position=UDim2.new(0,10,0,0) l.ZIndex=12
...
addSlider  (pgAim,"Smoothing (1=fast)",  "AimSmoothing",1,20)
addSection (pgAim,"INFO")
addInfo    (pgAim,"Toggle ON = always active, no hold")
addInfo    (pgAim,"R6 and R15 supported")
...
-- SETTINGS TAB
addSection (pgSet,"AIMBOT SETTINGS")
addSlider  (pgSet,"Aim FOV",              "AimFOV",20,500)
addSlider  (pgSet,"Smoothing",            "AimSmoothing",1,20)
addSection (pgSet,"ESP SETTINGS")
addSlider  (pgSet,"Max Distance (studs)", "MaxDist",100,2000)
addSection (pgSet,"ABOUT")
addInfo    (pgSet,"ESP: all BaseParts — works R6 + R15")
        ...
        if Cfg.ESPEnabled and player ~= LocalPlayer then
            ...
            if char and hum and hum.Health > 0 then
                ...
                if found and dist3 <= Cfg.MaxDist then
                    ...
                    local lFrom = center
                    if Cfg.ESPLinePos=="Top" then lFrom=Vector2.new(center.X,0)
                    ...
                    if Cfg.ESPHealthBarPos=="Left" then
                        o.hbg.Position=Vector2.new(bx-BT-2,by)   o.hbg.Size=Vector2.new(BT,bh)
                        o.hbar.Position=Vector2.new(bx-BT-2,by+bh*(1-hp)) o.hbar.Size=Vector2.new(BT,bh*hp)
                    elseif Cfg.ESPHealthBarPos=="Right" then
                        o.hbg.Position=Vector2.new(bx+bw+2,by)   o.hbg.Size=Vector2.new(BT,bh)
                        o.hbar.Position=Vector2.new(bx+bw+2,by+bh*(1-hp)) o.hbar.Size=Vector2.new(BT,bh*hp)
                    elseif Cfg.ESPHealthBarPos=="Top" then
                        o.hbg.Position=Vector2.new(bx,by-BT-2)   o.hbg.Size=Vector2.new(bw,BT)
                        o.hbar.Position=Vector2.new(bx,by-BT-2)  o.hbar.Size=Vector2.new(bw*hp,BT)
                    else
                        o.hbg.Position=Vector2.new(bx,by+bh+2)   o.hbg.Size=Vector2.new(bw,BT)
                        o.hbar.Position=Vector2.new(bx,by+bh+2)  o.hbar.Size=Vector2.new(bw*hp,BT)
                    end
                    ...
                    -- DISTANCE
                    local nOff=(Cfg.ESPName and Cfg.ESPNamePos=="Bottom") and 14 or 0
                    ...
                    o.dist.Position = Cfg.ESPDistancePos=="Top"
                        ...
                        or  Vector2.new(bx+bw/2, by+bh+2+nOff)
                end
            end
        end
        if not show then for _,d in pairs(o) do d.Visible=false end end
    end
end)

print("[ACMenu v4] Menu loaded at top-left. ESP + Aimbot ready.")
