-- MOBILE COMPLETE STRESS-TESTING SUITE (ALL FEATURES RESTORED)
-- Features: Non-Scoped Head Lock, Skeleton ESP, Distance Tracking, Health Bars, Floating Menu
...
local Options = {
    Aimbot = false, SilentAim = false, FOVCircle = false, FOVRadius = 150,
...
-- ==========================================
-- RIVALS MATRIX CAM & MOUSE REDIRECT HOOKS
-- ==========================================
local oldIndex
oldIndex = hookmetamethod(game, "__index", function(self, index)
    if not checkcaller() then
        if Options.SilentAim and self == LocalPlayer:GetMouse() then
        ...
        elseif Options.Aimbot and self == Camera and index == "CFrame" then
            local target = GetClosestPlayer()
            if target and target.Character and target.Character:FindFirstChild("Head") then
...
CreateQuickToggle("Head Lock (No Scope)", "Aimbot")
CreateQuickToggle("Silent Aim Bypass", "SilentAim")
...
CombatTab:CreateToggle({ Name = "Direct Head Lock (No-Scope)", CurrentValue = false, Callback = function(v) Options.Aimbot = v end })
CombatTab:CreateToggle({ Name = "Silent Aim Engine Bypass", CurrentValue = false, Callback = function(v) Options.SilentAim = v end })
...
RunService.RenderStepped:Connect(function()
    ...
    -- Direct Perspective Lock Loop (Bypasses Scopes)
    if Options.Aimbot and not Options.SilentAim then
        local target = GetClosestPlayer()
        if target and target.Character and target.Character:FindFirstChild("Head") then
            ...
            if Options.EspMaster and player.Character and player.Character:FindFirstChild("HumanoidRootPart") and player.Character:FindFirstChildOfClass("Humanoid") and player.Character:FindFirstChildOfClass("Humanoid").Health > 0 then
                ...
                if topOnScreen and bottomOnScreen then
                    if not Cache[player] then
                        ...
                        -- Pre-instantiate drawing vectors for player skeleton limits
